...
public class TutorViewSessionListModel {
    ...
    public List<TutorSession> fetchSessions(String tutorID) {
        try {
            System.out.println("[CLIENT | "+ new Date()+ "] Fetching sessions for tutor ID: " + tutorID);
            List<TutorSession> sessions = tutorService.viewSessionList(tutorID);
            System.out.println("[CLIENT | "+ new Date()+ "] Sessions fetched: " + (sessions != null ? sessions.size() : 0));
            return sessions;
        } catch (Exception e) {
